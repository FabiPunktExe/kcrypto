...
actual open class AES_GCM actual constructor(
    ...
    private fun encrypt(data: ByteArray, key: ByteArray, salt: ByteArray, iv: ByteArray): ByteArray {
        memScoped {
            val aesAlgHandle = alloc<BCRYPT_ALG_HANDLEVar>()
            ...
            try {
                ...
                BCryptCloseAlgorithmProvider(aesAlgHandle.value, 0u)
            }
        }
    }
    ...
    @OptIn(ExperimentalForeignApi::class)
    private fun decrypt(encryptedData: ByteArray, derivedKey: ByteArray, salt: ByteArray, iv: ByteArray, tag: ByteArray): ByteArray {
        memScoped {
            val aesAlgHandle = alloc<BCRYPT_ALG_HANDLEVar>()
    ...
    fun deriveKey(password: ByteArray, salt: ByteArray): ByteArray {
        ...
        memScoped {
            val sha256AlgHandle = alloc<BCRYPT_ALG_HANDLEVar>()
            var status = BCryptOpenAlgorithmProvider(
                sha256AlgHandle.ptr,
                BCRYPT_SHA256_ALGORITHM,
                null,
                BCRYPT_ALG_HMAC_FLAG
            )
            check(status == 0) { "BCryptOpenAlgorithmProvider failed: 0x${status.toHexString()}" }

            try {
                status = password.usePinned { pinnedPassword ->
                ...
                BCryptCloseAlgorithmProvider(sha256AlgHandle.value, 0u)
            }
        }

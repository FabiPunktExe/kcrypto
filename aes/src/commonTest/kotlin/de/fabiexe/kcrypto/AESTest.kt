...
class AESTest {
    @Test
    fun testGCM256() = runTest {
        val data = "foo bar"
        val password = "password"

        var encrypted = AES.GCM256.encrypt(data, password)
        var decrypted = AES.GCM256.decryptToString(encrypted, password)
        ...
        decrypted = AES.GCM256.decryptToString(encrypted, password)
        assertEquals(data, decrypted)
    }
}